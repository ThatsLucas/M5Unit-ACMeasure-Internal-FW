...
void Mycallback_handler(void) {
    ...
    if (HLW8032_SumCheck == buffer[23]) {
        ...
        Current_Parameter_REG =
            buffer[8] * 65536 + buffer[9] * 256 + buffer[10];
        Current_REG = buffer[11] * 65536 + buffer[12] * 256 + buffer[13];
        Power_Parameter_REG =
            buffer[14] * 65536 + buffer[15] * 256 + buffer[16];
        Power_REG = buffer[17] * 65536 + buffer[18] * 256 + buffer[19];

        DisplayTable[0] = (float)Voltage_Parameter_REG * voltage_factor_float /
                          Voltage_REG;  // 电压
        {
            DisplayTable[1] = (float)Current_Parameter_REG / Current_REG *
                              current_factor_float;  // 电流
            DisplayTable[2] = (float)Power_Parameter_REG *
                              voltage_current_factor / Power_REG;  // 功率
        ...
        DisplayTable[4] = (float)Energy_CNT / 1000000000 * Power_Parameter_REG *
                          voltage_current_factor / 3600;  // 电能
...
void Usart_Receive_Data(UART_HandleTypeDef *huart) {
    if (RESET !=
        __HAL_UART_GET_FLAG(huart, UART_FLAG_IDLE))  // 判断是否是空闲中断
    {
        __HAL_UART_CLEAR_IDLEFLAG(
            huart);  // 清除空闲中断标志（否则会一直不断进入中断）
