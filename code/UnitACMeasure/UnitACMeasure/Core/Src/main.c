...
void i2c1_receive_callback(uint8_t *rx_data, uint16_t len) {
    ...
    if (len == 1) {
        if ((rx_data[0] >= 0x60) && (rx_data[0] <= 0x61)) {
            ...
            if (uint32_dis_value > 100) uint32_dis_value = 100;
            ...
            temp = temp / 100;
            dec  = dec - temp * 100;
            if (temp < 9999) sprintf(temp_integer_part, "%04d", temp);
            if (dec < 99) sprintf(temp_decimal_part, "%c%02d", '.', dec);
            memcpy(temp_tx_char_1, temp_integer_part, 4);
            memcpy(temp_tx_char_1 + 4, temp_decimal_part, 3);
            i2c1_set_send_data((uint8_t *)temp_tx_char_1,
                               sizeof(temp_tx_char_1));
            ...
            i2c1_set_send_data((uint8_t *)temp_tx_char_2,
                               sizeof(temp_tx_char_2));
            ...
            temp = temp / 100;
            dec  = dec - temp * 100;
            if (temp < 99999999) sprintf(temp_integer_part, "%08d", temp);
            if (dec < 99) sprintf(temp_decimal_part, "%c%02d", '.', dec);
            memcpy(temp_tx_char_3, temp_integer_part, 8);
            memcpy(temp_tx_char_3 + 8, temp_decimal_part, 3);
            i2c1_set_send_data((uint8_t *)temp_tx_char_3,
                               sizeof(temp_tx_char_3));
        ...
        if (rx_data[0] == 0xC0) {
            ...
            if (len == 5) {
                uint32_t temp = 0;
                temp = (rx_data[1] | (rx_data[2] << 8) | (rx_data[3] << 16) |
                        (rx_data[4] << 24));
